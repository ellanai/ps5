...
def scrape_enforcement_actions(start_date):
    ...
    if start_date.year < 2013:
        print("Please enter a start date >= 2013")
        return
    ...
    while True:  # Loop through pages
        ...
        # Extract titles, dates, and links on the summary page
        titles = list(filter(lambda tag: tag.get('href') and '/fraud/enforcement/' in tag['href'], soup.find_all('a')))[3:]
        date_divs = soup.find_all('div', class_='font-body-sm margin-top-1')
        
        # Stop if the page has no new data
        if not titles:
            break
        
        page_has_new_data = False  # Flag to check if page has any valid actions
        ...
        for i in range(len(titles)):
            ...
            if action_date >= start_date:
                ...
                # Find the agency using your partner's method
                li_tags = agency_soup.find_all('li')
                for li in li_tags:
                    if 'Agency:' in li.get_text():
                        agency = li.get_text().split('Agency:')[-1].strip()
                        break
                ...
                # Mark this page as having new data to avoid breaking the loop
                page_has_new_data = True

        # If no valid actions were added, break the loop
